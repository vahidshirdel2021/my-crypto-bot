...
def _systemic_macro_divergence(context: ConfluenceContext, btc_context: dict = None) -> bool:
    ...
    صعودی است در حالی که ساختار ۱۵m بیت‌کوین (SDE) اخیراً BOS نزولی داده،
    جریمه اعمال شود (و برعکس برای anchor نزولی + BOS صعودی).

    توجه: این چک قبلاً فاز کلان MCDE (Wyckoff) بیت‌کوین را هم بررسی
    می‌کرد؛ طبق درخواست صریح کاربر MCDE به‌طور کامل حذف شده، پس این تابع
    اکنون فقط بر پایه‌ی ساختار SDE (`structure_direction_15m`) تصمیم
    می‌گیرد.

    btc_context (اختیاری، از بیرون تزریق می‌شود — طبق طراحی این پروژه که
    هیچ موتوری نباید نمادهای دیگر را مستقیماً بخواند): دیکشنری با کلید
    'structure_direction_15m' (str|None).
    """
    ...
    if anchor_dir == "bullish" and structure_dir == "bearish":
        return True
    if anchor_dir == "bearish" and structure_dir == "bullish":
        return True
    return False
