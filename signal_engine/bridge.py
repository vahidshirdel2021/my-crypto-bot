...
فعال‌سازی این پل پشت یک فلگ کانفیگ (`use_new_signal_engine`) است؛ طبق
تصمیم صریح کاربر (۲۰۲۶-۰۹-۰۸) این فلگ اکنون پیش‌فرض True است — یعنی
موتور قدیمی PDH/EQ/PDL دیگر برای هیچ سشنی صدا زده نمی‌شود مگر کاربر
صریحاً این فلگ را در `strategy.STRATEGY_DEFAULTS` به False برگرداند.
"""
...
def _construct_entry_sl_tp(
    ...
    structural_target_ref = _best_reference_price(
