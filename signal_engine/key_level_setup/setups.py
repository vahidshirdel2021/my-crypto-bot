...
تعاریف سند هستند، نه پیاده‌سازی‌های «کامل» موتور تشخیص کندل‌استیک
جداگانه‌ای که سند به‌عنوان یکپارچگی نهایی (بخش ۱۰) توصیه می‌کرد — طبق
درخواست صریح کاربر آن موتور (CPDE) هرگز ساخته نشد و کلاً از پروژه حذف
شده، پس این تقریب‌ها نهایی هستند، نه موقت.
"""
