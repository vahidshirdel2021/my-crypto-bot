...
def compute_candle_geometry(
    ...
    *,
    ...
    override به‌ازای هر کالر — مثلاً می‌توان doji_threshold را
    per-instrument (بر اساس نوسان‌پذیری واقعی نماد) تنظیم کرد، دقیقاً طبق
...
def compute_candle_geometry_batch(
    ...
    *,
    ...
    وقتی که یک موتور نیاز به هندسه‌ی *همه‌ی* کندل‌های یک سری دارد (مثلاً
    برای چک کردن هر کندل که آیا چکش/دوجی است). طبق پروفایل واقعی عملکرد
    این پروژه، فراخوانی نسخه‌ی اسکالر (تک‌کندلی) به ازای هر ردیف یک df
    بزرگ، گلوگاه اصلی بود؛ این تابع همان محاسبات را یک‌بار با numpy روی
    کل سری انجام می‌دهد.
