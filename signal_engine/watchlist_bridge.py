...
def compute_btc_context(btc_df, timeframe: str = "15m") -> Optional[dict]:
    """طبق بخش ۴ سند اصلاحی (Architectural Addendum): «USCL باید ساختار
    ۱۵ دقیقه‌ای بیت‌کوین (SDE) را پایش کند» — این تابع واقعاً همان موتور
    را روی داده‌ی خودِ BTC اجرا می‌کند (نه یک هیوریستیک EMA/ADX سبک‌تر
    مثل leader_correlation_guard موجود در bot.py که برای هدف دیگری
    طراحی شده).

    توجه: این تابع قبلاً فاز کلان Wyckoff بیت‌کوین (MCDE) را هم محاسبه
    می‌کرد؛ طبق درخواست صریح کاربر MCDE به‌طور کامل از پروژه حذف شده،
    پس این تابع اکنون فقط SDE را اجرا می‌کند.

    خروجی: {'structure_direction_15m': str|None} یا None اگر داده کافی
    نبود یا ساختاری یافت نشد (طبق اصل fail-open: نبود btc_context یعنی
    ...
    if structure_direction is None:
        return None
    return {"structure_direction_15m": structure_direction}
