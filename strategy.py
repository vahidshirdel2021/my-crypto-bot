...
def get_signal_with_reason(df_primary, market_data_dict=None, timeframe_mode="single", timeframe="5min", strategy_type="trend", filters=None, strategy_config=None, regime=None, live_price=None):
    ...
    # «محافظ خلاف‌جهت بازار» که این‌جا بود حذف شد. فیلتر جهت‌دار رژیم دیگر روی خروجی
    # استراتژی اعمال نمی‌شود - فقط یک‌بار، پیش از اسکن، روی انتخاب نمادهای واچ‌لیست اعمال
    # می‌شود (bot.py::filter_watchlist_by_market_regime). پارامتر regime برای سازگاری
    # امضای تابع نگه داشته شده ولی دیگر مصرف نمی‌شود.
    return sig, reason
