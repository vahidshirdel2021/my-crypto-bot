...
# به‌زودی با «Swing Detection & Market Structure Engine» جدید (بر اساس
# فراکتال + فیلتر نویز نرمال‌شده با ATR + BOS/CHoCH) جایگزین خواهد شد.
# تا آن زمان، compute_swing_stop_v2 پایین همیشه (None, None) برمی‌گرداند و
# bot.py طبق طراحی از قبل موجودش به‌صورت ایمن به compute_swing_stop قدیمی
# (پنجره‌ی ثابت lookback) فال‌بک می‌کند — بدون هیچ تغییر رفتاری محسوس.
...
STRATEGY_DEFAULTS = {
    # --- فلگ مهاجرت به معماری جدید (signal_engine: موتورهای SDE و KLSDE +
    # لایه‌ی تجمیع سیگنال؛ سه موتور PRE/CPDE/MCDE قبلی طبق درخواست صریح
    # کاربر کاملاً از پروژه حذف شده‌اند) — طبق تصمیم صریح کاربر (۲۰۲۶-۰۹-۰۸):
    # برای اعتبارسنجی روی پیپر تریدینگ، این فلگ اکنون پیش‌فرض **روشن** است؛
    # یعنی موتور قدیمی PDH/EQ/PDL (evaluate_scenarios) دیگر برای هیچ سشنی
    # صدا زده نمی‌شود مگر این‌که این فلگ صریحاً به False برگردانده شود.
    # وقتی True است، _run_engine_multi_source به‌جای evaluate_scenarios قدیمی
    ...
    # جدید هم اجرا می‌شوند. توجه: چون normalize_session در bot.py همیشه
    # strategy_config هر سشن را از روی همین STRATEGY_DEFAULTS دوباره
    # می‌سازد (get_timeframe_preset)، این تغییر خودکار روی همه‌ی سشن‌های
    # موجود (حتی آن‌هایی که قبلاً با فلگ False ذخیره شده بودند) هم اعمال
    # می‌شود، نه فقط سشن‌های تازه.
