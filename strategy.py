...
def get_signal_with_reason(df_primary, market_data_dict=None, timeframe_mode="single", timeframe="5min", strategy_type="trend", filters=None, strategy_config=None, regime=None, live_price=None):
    ...
    # --- محافظ خلاف‌جهت بازار (نسخه‌ی خانواده‌آگاه) ---
    # این فیلتر روی خروجی *همه‌ی* مسیرهای بالا (شامل dynamic V2 که مسیر پیش‌فرض ربات است)
    # اعمال می‌شود - قبلاً مسیر V2 زودتر از این نقطه return می‌کرد و این فیلتر را دور
    # می‌زد؛ این نقص برطرف شد.
    ...
    # فیلتر هم فعال است. در حالت رنج/نامشخص (نه اکثریت صعودی نه نزولی) هیچ تاثیری ندارد.
    #
    # نکته‌ی مهم (تصمیم آگاهانه، نه باگ): خانواده‌ی Liquidity Sweep ذاتاً یک استراتژی
    # *برگشتی* است - کارش دقیقاً گرفتن سویپ+ریکلیم روی سطوح مهم (روزانه/هفتگی/ماهانه)
    # است، حتی وقتی آن سطح خلاف روند غالب لحظه‌ای باشد. اعمال این فیلتر روی همین خانواده
    # دقیقاً همان نوع معامله‌ای را که استراتژی برای شکار آن طراحی شده حذف می‌کند - یک
    # تناقض ساختاری. برای همین این خانواده از این فیلتر *معاف* است؛ حفاظتش را
    # leader_correlation_guard (که بر اساس همبستگی واقعی و شدت حرکت لیدرهاست، نه صرفاً
...
V1_ENHANCED_DEFAULTS = {
    ...
    "enhanced_use_advanced_swings": True,
    "enhanced_use_explicit_scenarios": True,
    ...
    "enhanced_swing_right": 3,
    "enhanced_swing_min_wick_atr": 0.12,
...
def get_v2_config(strategy_config=None):
    cfg = {**STRATEGY_DEFAULTS, **V2_DEFAULTS, **V1_ENHANCED_DEFAULTS}
