...
def _force_paper_test_open(chat_id, symbol, side, price, sl, tp):
    """SAFE diagnostic: open one PAPER position without strategy/exchange entry gates."""
    s=get_session(chat_id)
    if s.get('paper_positions'):
        return False, 'already_has_paper_position'
    if not s.get('is_bot_active') or s.get('daily_stopped'):
        return False, 'bot_inactive_or_daily_stopped'
    price=float(price)
    margin=float(s.get('trade_amount_usdt') or 50.0)
    balance=float(s.get('paper_balance') or 0.0)
    if margin <= 0 or balance < margin:
        return False, f'insufficient_paper_balance balance={balance:.2f} margin={margin:.2f}'
    leverage=max(1,int(s.get('leverage') or 1))
    trade_id=new_trade_id(chat_id,symbol)
    risk_dist=abs(price-float(sl))
    trade={'trade_id':trade_id,'setup_id':f'FORCE_TEST:{symbol}:{int(time.time())}','symbol':symbol,'side':side,'entry_price':price,'sl':float(sl),'tp':float(tp),'margin':margin,'leverage':leverage,'amount':(margin*leverage)/max(price,1e-12),'timeframe':s.get('timeframe','5min'),'strategy':'FORCE_PAPER_TEST','is_real':False,'paper_slippage_bps':0.0,'paper_funding_rate_pct_8h':0.0,'opened_at':time.time(),'signal_reason':'FORCE_PAPER_TEST | NO FILTERS','entry_reason':'FORCE_PAPER_TEST | NO FILTERS','risk_pct':float(s.get('risk_per_trade_pct') or 0),'risk_usdt':abs(price-float(sl))/max(price,1e-12)*margin*leverage,'quality_score':100.0,'quality_label':'TEST','planned_rr':2.0,'mfe_usdt':0.0,'mae_usdt':0.0,'mfe_r':0.0,'mae_r':0.0,'peak_favorable_price':None,'peak_adverse_price':None,'last_price':price,'duration_seconds':0.0,'realized_r':None,'trailing_activated':False,'risk_distance':risk_dist,'trailing_locked_r':0.0,'swing_sl_level':None}
    s['trading_mode']='PAPER'
    s['paper_positions'].append(trade)
    save_session(chat_id)
    audit_event(chat_id,trade_id,'signal_and_plan',{'symbol':symbol,'side':side,'signal_price':price,'sl':sl,'tp':tp,'reason':trade['signal_reason'],'test':True})
    audit_event(chat_id,trade_id,'paper_opened',{'entry_price':price,'amount':trade['amount'],'margin':margin,'test':True})
    audit_event(chat_id,trade_id,'position_opened',audit_trade_record(trade))
    logger.warning('FORCE_PAPER_TEST PAPER_OPENED chat=%s symbol=%s side=%s entry=%s sl=%s tp=%s margin=%s',chat_id,symbol,side,price,sl,tp,margin)
    try:
        send_message(chat_id, f'🧪 FORCE TEST — پوزیشن PAPER باز شد\nنماد: `{symbol}` | جهت: `{"LONG" if side_long(side) else "SHORT"}`\nورود: `{fmt(price)}` | SL: `{fmt(sl)}` | TP: `{fmt(tp)}`\nمارجین: `{margin:.2f}` USDT\n✅ بدون فیلتر استراتژی و بدون CoinEx')
    except Exception:
        pass
    return True,'paper_opened'
    ...
    if UNFILTERED_TEST:
        try:
            ...
            if side == 'BUY':
                ...
                sl = price + risk_dist; tp = price - 2.0 * risk_dist
            reason = f'FORCE_PAPER_TEST | {side} | Quality=100 | RR=2.00 | NO FILTERS'
            ok, detail = _force_paper_test_open(chat_id, symbol, 'BUY (Long)' if side == 'BUY' else 'SELL (Short)', price, sl, tp)
            return _entry_diag_result(chat_id, symbol, 'entry_opened' if ok else 'execute_blocked',
                                      reason if ok else f'FORCE_PAPER_TEST blocked: {detail}', 'force_paper_test', side)
        except Exception as exc:
