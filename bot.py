...
# طبق بخش ۴ سند اصلاحی (Architectural Addendum): وضعیت ساختار ۱۵دقیقه‌ای
# BTC (SDE) — فقط یک‌بار در هر بازه‌ی TTL محاسبه می‌شود (نه به‌ازای هر
# نماد در هر اسکن) چون همه‌ی نمادها در یک چرخه‌ی اسکن از همان یک وضعیت
# BTC مقایسه می‌شوند. (فاز کلان MCDE قبلاً هم اینجا محاسبه می‌شد؛ طبق
# درخواست صریح کاربر MCDE به‌طور کامل از پروژه حذف شده است.)
...
async def _get_btc_context_cached(http):
    """طبق بخش ۴ سند اصلاحی: یک‌بار در هر ۵ دقیقه، SDE واقعی روی داده‌ی
    خودِ BTC اجرا می‌شود (نه هیوریستیک سبک‌تر leader_correlation_guard
    ...
    واگرایی سیستمیک را برای کل یک چرخه‌ی اسکن خاموش نکند. (قبلاً MCDE هم
    اینجا اجرا می‌شد؛ طبق درخواست صریح کاربر MCDE حذف شده است.)
    """
    now = time.time()
