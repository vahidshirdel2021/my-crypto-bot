...
from signal_engine.key_level_setup.levels import min_klines_for_levels, get_reference_levels
from ui import (
...
DEX_WATCHLIST_CACHE = {
    ...
    'changes': [],
    # dollar_volume: {symbol: حجم دلاری ۲۴ساعته} — از همین فراخوانی CoinGecko،
    # برای واچ‌لیست هوشمند (signal_engine.watchlist) بدون فراخوانی اضافه.
    'dollar_volume': {},
}
# طبق بخش ۴ سند اصلاحی (Architectural Addendum): وضعیت فاز کلان BTC
# (MCDE) و ساختار ۱۵دقیقه‌ای‌اش (SDE) — فقط یک‌بار در هر بازه‌ی TTL
# محاسبه می‌شود (نه به‌ازای هر نماد در هر اسکن) چون همه‌ی نمادها در یک
# چرخه‌ی اسکن از همان یک وضعیت BTC مقایسه می‌شوند.
_BTC_CONTEXT_CACHE = {'ts': 0.0, 'context': None}
_BTC_CONTEXT_CACHE_TTL_SECONDS = 300
...
def normalize_session(data):
    ...
    for k in legacy_flat_keys:
        s.pop(k, None)
    # کلید قدیمی موتور حذف‌شده B1-B7/S1-S7 را نیز از سشن‌های ذخیره‌شده‌ی قدیمی پاک می‌کنیم.
    s.pop('disabled_setups', None)
    # موتور «اکسترا» (Killzone/Judas Swing/MSS) حذف شده؛ سشن‌های قدیمی که هنوز
    # روی این حالت مانده‌اند را به‌صورت امن به استراتژی پویای پیش‌فرض برمی‌گردانیم.
    if s.get('active_strategy') == 'extra':
        s['active_strategy'] = 'dynamic'
        ...
        if r.ok:
            ...
            if ranked_full:
                ...
                DEX_WATCHLIST_CACHE.update({
                    ...
                    'changes': changes[-100:],
                    # طبق مستندات signal_engine.watchlist: حجم دلاری برای فیلتر سخت/امتیاز
                    # نقدینگی واچ‌لیست هوشمند — همان فراخوانی CoinGecko، بدون هزینه‌ی اضافه.
                    'dollar_volume': dollar_volume_by_symbol,
                })
                return list(kept)
    except Exception as exc:
...
def scan_watchlist_for_timeframe(timeframe, regime=None):
    ...
    try:
        ...
        if is_adaptive_watchlist_enabled():
            ...
            symbols = select_symbols_for_cycle(symbols, dv_map)
    except Exception as exc:
...
def close_position(chat_id,pos,price=None,reason='manual'):
    ...
    setup_line = f"\n• ستاپ: `{pos['scenario_code']}`" if pos.get('scenario_code') else ''
    send_message(chat_id,f"📌 *پوزیشن {'REAL' if pos.get('is_real') else 'PAPER'} بسته شد*\n• `{pos['symbol']}`\n• خروج: `{fmt(pos['close_price'])}`\n• PnL خالص کل معامله{est}: `{total_pnl:+.2f} USDT`{partials_line}{fee_line}{platform_line}{setup_line}\n• علت: `{reason}`")
    return True
...
def reconcile_real(chat_id):
    ...
    for sym,p in list(known.items()):
        if sym in live: p.update(live[sym])
            ...
            setup_line = f"\nستاپ: `{p['scenario_code']}`" if p.get('scenario_code') else ''
            send_message(chat_id,f"📌 پوزیشن REAL `{sym}` توسط صرافی بسته شد.\nPnL ثبت‌شده: `{p['pnl_usdt']:+.2f} USDT`{setup_line}")
...
def update_positions(chat_id):
    ...
    for p in s['paper_positions'][:]:
        ...
        # طبق بخش ۲ سند اصلاحی (Architectural Addendum): چک ابطال ساختاری —
        # فقط برای رصد/آدیت (log-only)، هیچ پوزیشنی را نمی‌بندد و هیچ SL ای
        # را جابه‌جا نمی‌کند. چون این بات سفارش را بلافاصله در لحظه‌ی تشخیص
        # سیگنال اجرا می‌کند (نه سفارش معلق منتظر پر شدن)، سناریوی دقیق سند
        # («لغو سفارش معلق») این‌جا صادق نیست؛ این فقط یک قدم محافظه‌کارانه‌ی
        # اول برای دیده‌شدن این رویداد است، پیش از هر تصمیم درباره‌ی تبدیل آن
        # به یک محرک خروج فعال.
        if s.get('strategy_config', {}).get('use_new_signal_engine'):
            try:
                from signal_engine.bridge import check_signal_invalidation
                inv_event = check_signal_invalidation(p['symbol'], float(price), int(time.time()))
                if inv_event:
                    logger.warning('signal_invalidated (audit-only, no action taken) chat=%s %s', chat_id, inv_event)
            except Exception:
                pass
...
async def _get_btc_context_cached(http):
    """طبق بخش ۴ سند اصلاحی: یک‌بار در هر ۵ دقیقه، MCDE/SDE واقعی روی
    داده‌ی خودِ BTC اجرا می‌شود (نه هیوریستیک سبک‌تر leader_correlation_guard
    که برای هدف دیگری طراحی شده) و نتیجه برای همه‌ی نمادهای همین چرخه‌ی
    اسکن به اشتراک گذاشته می‌شود. اگر فچ/محاسبه شکست بخورد، مقدار قبلی
    (نه None ناگهانی) نگه داشته می‌شود تا یک گلیچ لحظه‌ای، جریمه‌ی
    واگرایی سیستمیک را برای کل یک چرخه‌ی اسکن خاموش نکند.
    """
    now = time.time()
    if now - _BTC_CONTEXT_CACHE['ts'] < _BTC_CONTEXT_CACHE_TTL_SECONDS:
        return _BTC_CONTEXT_CACHE['context']
    ctx = _BTC_CONTEXT_CACHE['context']
    try:
        d = await get_klines_async(http, 'BTC', '15min', 300)
        if d is not None and not d.empty and len(d) >= 20:
            from signal_engine.watchlist_bridge import compute_btc_context
            new_ctx = compute_btc_context(calculate_indicators(d), timeframe='15m')
            if new_ctx is not None:
                ctx = new_ctx
    except Exception as exc:
        logger.warning('BTC context (signal_engine addendum) fetch failed: %s', exc)
    _BTC_CONTEXT_CACHE['ts'] = now
    _BTC_CONTEXT_CACHE['context'] = ctx
    return ctx
...
async def scan_symbol(http,chat_id,symbol,regime=None):
    ...
    # طبق بخش ۱ سند اصلاحی (LMP): از همین کندل‌هایی که همین الان برای
    # تولید سیگنال fetch شدند (بدون هیچ فراخوانی API اضافه) برای تشخیص
    # ناهنجاری ارزان استفاده می‌کنیم. محدودیت صادقانه: این نسخه‌ی MVP
    # فقط نمادهایی را می‌بیند که در همین چرخه انتخاب به اسکن شده‌اند
    # (نه *کل* یونیورس هم‌زمان، که نیازمند یک فچ سبک جداگانه برای صدها
    # نماد در هر چرخه است — یک سرمایه‌گذاری زیرساختی جدا)؛ اما همچنان
    # ارزش اصلی را می‌دهد: نمادی که حتی به‌ندرت اسکن می‌شود و در همان
    # اسکن نادر یک ناهنجاری نشان دهد، برای چرخه‌ی بعد سریع دیده می‌شود.
    try:
        from signal_engine.watchlist_bridge import is_adaptive_watchlist_enabled, get_global_watchlist
        if is_adaptive_watchlist_enabled():
            from signal_engine.watchlist.lmp import detect_pulse
            trig = detect_pulse(primary, symbol)
            if trig is not None:
                tf_minutes = {'5min': 5, '15min': 15, '1hour': 60, '4hour': 240}.get(tf, 5)
                get_global_watchlist().handle_market_pulse(symbol, trig, len(primary), tf_minutes)
    except Exception as exc:
        logger.warning('LMP check skipped for %s: %s', symbol, exc)
    ...
    # طبق بخش ۴ سند اصلاحی (Architectural Addendum): فقط وقتی موتور جدید
    # فعال است (فراخوانی اضافه‌ی BTC هیچ فایده‌ای برای موتور قدیمی ندارد)
    # و خودِ نماد BTC/ETH نیست (طبق تعریف سند، این جریمه فقط برای آلت‌کوین‌ها
    # نسبت به BTC معنا دارد).
    if effective_strategy_config.get('use_new_signal_engine') and symbol.upper() not in LEADER_SYMBOLS:
        try:
            btc_ctx = await _get_btc_context_cached(http)
            if btc_ctx:
                effective_strategy_config['btc_context'] = btc_ctx
                effective_strategy_config['asset_taxonomy'] = {'asset_class': 'crypto_perp'}
        except Exception as exc:
            logger.warning('btc_context injection skipped for %s: %s', symbol, exc)
    if effective_strategy_config.get('use_new_signal_engine'):
        effective_strategy_config['watchlist_symbol'] = symbol
...
def handle_text(chat_id,text):
    ...
    fixed_buttons={
        ...
        '🟢 اسکن بازار فعال':'/stop_scan', 'اسکن بازار فعال':'/stop_scan',
        '🔴 اسکن بازار خاموش':'/start_scan', 'اسکن بازار خاموش':'/start_scan',
    }
...
async def _send_periodic_heartbeat():
    ...
    for cid in due:
        ...
        try:
            balance = exchange_balance(cid) if sess.get('trading_mode') == 'REAL' else float(sess.get('paper_balance', 0.0) or 0.0)
        except Exception:
            balance = float(sess.get('paper_balance', 0.0) or 0.0)
        open_positions = sess.get('paper_positions', []) or []
        pos_count = len(open_positions)
        # PnL لحظه‌ای پوزیشن‌های باز از last_unrealized_pnl خوانده می‌شود که
        # همین‌الان در scan_loop (قبل از فراخوانی این تابع) توسط update_positions
        # به‌روز شده — پس نیازی به محاسبه‌ی دوباره نیست.
        open_pnl = sum(float(p.get('last_unrealized_pnl') or 0.0) for p in open_positions)
        pnl_suffix = f" (PnL لحظه‌ای: {open_pnl:+.2f} USDT)" if pos_count else ""
        message = (
            HEARTBEAT_TEXT + "\n\n"
            f"📊 وضعیت کلی بازار ( براساس داشبورد بازار ): {regime_label}\n\n"
            f"💰 قیمت لحظه‌ای BTC: {price_txt}\n\n"
            f"💳 موجودی اکانت: {balance:,.2f} USDT\n\n"
