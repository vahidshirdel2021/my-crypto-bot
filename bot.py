...
# قفل سود پله‌ای: PROFIT_LOCK_USDT هم آستانه‌ی اول و هم اندازه‌ی هر پله است. به‌محض رسیدن سود
# لحظه‌ای هر معامله به 5، 10، 15، ... دلار، حد ضرر روی نقطه‌ای می‌رود که خروج دقیقاً همان
# پله را سود بدهد. اگر قیمت ادامه بدهد پله‌ی بعدی قفل می‌شود؛ اگر برگردد، با آخرین پله‌ی
# قفل‌شده بسته می‌شود. (TP و تریلینگ‌های موجود دست‌نخورده می‌مانند.)
...
def _apply_profit_lock(chat_id, s, p, price):
    """قفل سود پله‌ای (پله = PROFIT_LOCK_USDT دلار). هر بار که سود لحظه‌ای (با قیمت زنده) از پله‌ی
    بالاتری عبور کند، SL به قیمتی می‌رود که خروج دقیقاً همان پله را سود بدهد (5، 10، 15، ...).
    اگر سود یک‌باره چند پله بپرد، بالاترین پله‌ی رسیده‌شده قفل می‌شود. SL هرگز بازتر نمی‌شود؛
    بقیه‌ی مکانیزم‌ها (TP، پلکان R، سوینگ) فقط می‌توانند آن را به نفع پوزیشن جلوتر ببرند.
    برمی‌گرداند آیا SL جابه‌جا شد."""
    if not PROFIT_LOCK_ENABLED or PROFIT_LOCK_USDT <= 0:
        return False
    try:
        ...
        if entry <= 0 or notional <= 0 or not math.isfinite(notional):
            return False
        is_long = side_long(p['side'])
        step = float(PROFIT_LOCK_USDT)
        fee = round_trip_fee_usdt(p.get('margin'), p.get('leverage')) if PROFIT_LOCK_NET_OF_FEES else 0.0
        frac = (price - entry) / entry if is_long else (entry - price) / entry
        profit = notional * frac - fee  # سود در همان مقیاسی که پله‌ها تعریف شده‌اند
        level = math.floor(profit / step + 1e-9) * step
        locked = float(p.get('profit_lock_usdt') or 0.0)
        if p.get('profit_lock_active') and locked <= 0:
            locked = step  # سازگاری با پوزیشن‌هایی که با نسخه‌ی تک‌پله‌ای قفل شده بودند
        if level < step or level <= locked + 1e-9:
            return False
        target = level + fee
        ...
        if already_better:
            # مکانیزم دیگری قبلاً سود بیشتری قفل کرده؛ فقط پله را ثبت می‌کنیم که دوباره بررسی نشود.
            p['profit_lock_active'] = True
            p['profit_lock_usdt'] = float(level)
            return False
        if p.get('is_real'):
            ok, err = move_stop_loss(chat_id, p['symbol'], new_sl)
            if not ok:
                logger.warning('profit-lock SL move failed symbol=%s: %s', p.get('symbol'), err)
                return False  # پله ثبت نمی‌شود؛ چرخه‌ی بعد دوباره تلاش می‌شود
        p['sl'] = new_sl
        p['sl_moved_ts'] = time.time()
        p['profit_lock_active'] = True
        p['profit_lock_usdt'] = float(level)
        p['profit_lock_sl'] = float(new_sl)
        send_message(chat_id, f"🔒 قفل سود {level:g} دلار فعال شد: `{p['symbol']}`\n• حد ضرر جدید: `{fmt(new_sl)}` (قبلی: `{fmt(old_sl)}`)\n• اگر قیمت برگردد همین‌جا با سود بسته می‌شود؛ اگر ادامه بدهد پله‌ی بعدی ({level + step:g} دلار) قفل می‌شود.")
        return True
    except Exception as exc:
