...
def process_command(cmd,chat_id,message_id=None):
    ...
    if cl in ('/manual_trade','🖐 معامله دستی'):
        ...
        manual_url = miniapp_manual_trade_url()
        kb = {'inline_keyboard': [[{'text': '🧾 باز کردن فرم معامله دستی', 'web_app': {'url': manual_url}}]]} if manual_url else None
        send_message(chat_id,
            '🖐 *معامله دستی*\n\n'
            + ('می‌تونید از فرم گرافیکی زیر استفاده کنید، یا اطلاعات رو تو یه پیام متنی بفرستید:\n\n' if manual_url else 'اطلاعات رو تو یه پیام، هرکدوم تو یه خط، به همین ترتیب بفرستید:\n\n')
            + '`جهت (خرید/فروش)`\n`نماد`\n`قیمت ورود (یا بازار)`\n`حد سود`\n`حد ضرر`\n\n'
              'مثال:\n`خرید`\n`BTC`\n`بازار`\n`67000`\n`64000`',
            kb
        ); return
...
_MINIAPP_MANUAL_TRADE_HTML = """<!DOCTYPE html>
<html lang="fa" dir="rtl">
<head>
<meta charset="utf-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"/>
<title>معامله دستی</title>
<script src="https://telegram.org/js/telegram-web-app.js"></script>
<style>
  :root{
    --bg: var(--tg-theme-bg-color, #0e0e12);
    --sec-bg: var(--tg-theme-secondary-bg-color, #17171d);
    --text: var(--tg-theme-text-color, #eaeaea);
    --hint: var(--tg-theme-hint-color, #8a8a94);
    --link: var(--tg-theme-link-color, #6ab7ff);
    --btn: var(--tg-theme-button-color, #2ea6ff);
    --btn-text: var(--tg-theme-button-text-color, #ffffff);
    --buy: #26a69a; --sell: #ef5350; --border:#26262f;
  }
  *{box-sizing:border-box;}
  html,body{margin:0;padding:0;background:var(--bg);color:var(--text);font-family:-apple-system,Tahoma,sans-serif;}
  body{padding:16px 16px 90px 16px;}
  ...
  input[type=text], input[type=number]{
    ...
    background:var(--sec-bg);color:var(--text);font-size:16px;outline:none;
  }
  input:focus{border-color:var(--btn);}
  .side-row{display:flex;gap:10px;}
  .side-btn{
    flex:1;padding:12px;border-radius:10px;border:1px solid var(--border);background:var(--sec-bg);
    color:var(--text);font-size:15px;text-align:center;cursor:pointer;user-select:none;
  }
  .side-btn.active-buy{background:var(--buy);border-color:var(--buy);color:#fff;font-weight:600;}
  .side-btn.active-sell{background:var(--sell);border-color:var(--sell);color:#fff;font-weight:600;}
  .market-row{display:flex;align-items:center;gap:8px;margin-top:8px;font-size:13px;color:var(--hint);}
  #err{color:var(--sell);font-size:13px;min-height:18px;margin-top:6px;}
  #price-hint{font-size:12px;color:var(--hint);margin-top:6px;}
</style>
</head>
<body>
<h2>🖐 ثبت معامله دستی</h2>

<div class="field">
  <label>نماد</label>
  <input type="text" id="symbol" placeholder="مثلاً BTC" autocomplete="off"/>
...
<div class="field">
  ...
  <div class="side-row">
    <div class="side-btn" id="btn-buy">🟢 خرید (Long)</div>
    <div class="side-btn" id="btn-sell">🔴 فروش (Short)</div>
  </div>
</div>

<div class="field">
  <label>قیمت ورود</label>
  <input type="number" id="entry" inputmode="decimal" placeholder="خالی = قیمت بازار"/>
</div>

<div class="field">
  <label>حد سود (TP)</label>
  <input type="number" id="tp" inputmode="decimal" placeholder="مثلاً 67000"/>
</div>

<div class="field">
  <label>حد ضرر (SL)</label>
  <input type="number" id="sl" inputmode="decimal" placeholder="مثلاً 64000"/>
</div>
...
btnBuy.onclick = () => { side='BUY'; btnBuy.classList.add('active-buy'); btnSell.classList.remove('active-sell'); };
btnSell.onclick = () => { side='SELL'; btnSell.classList.add('active-sell'); btnBuy.classList.remove('active-buy'); };

function setMainButton(){
  if (!tg) return;
  tg.MainButton.setText('✅ ثبت معامله');
...
function showError(msg){
  ...
  if (tg) { if (msg) tg.MainButton.enable(); }
}

async function submit(){
  showError('');
  const symbol = (document.getElementById('symbol').value || '').trim().toUpperCase();
  const entryRaw = (document.getElementById('entry').value || '').trim();
  const tpRaw = (document.getElementById('tp').value || '').trim();
  const slRaw = (document.getElementById('sl').value || '').trim();

  if (!symbol) { showError('نماد را وارد کنید.'); return; }
  if (!side) { showError('جهت معامله (خرید/فروش) را انتخاب کنید.'); return; }
  if (!tpRaw || !slRaw) { showError('حد سود و حد ضرر الزامی است.'); return; }
  const tp = parseFloat(tpRaw), sl = parseFloat(slRaw);
  if (isNaN(tp) || isNaN(sl)) { showError('حد سود/ضرر باید عدد باشد.'); return; }
  let entry = null;
  if (entryRaw) {
    entry = parseFloat(entryRaw);
    if (isNaN(entry)) { showError('قیمت ورود باید عدد باشد یا خالی (=بازار).'); return; }
    const isLong = side === 'BUY';
    const ok = isLong ? (sl < entry && entry < tp) : (tp < entry && entry < sl);
    if (!ok) { showError(`نسبت SL/TP/ورود با جهت ${isLong ? 'خرید' : 'فروش'} همخوانی ندارد.`); return; }
  }

  if (tg) tg.MainButton.showProgress();
  try {
    const res = await fetch('/miniapp/api/manual_trade', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        initData: tg ? tg.initData : '',
        symbol, side, entry: entryRaw ? entry : null, tp, sl
      })
    });
    const data = await res.json();
    if (tg) tg.MainButton.hideProgress();
    if (!res.ok || data.error) {
      showError(data.error || 'خطا در ثبت. دوباره تلاش کنید.');
      return;
    }
    if (tg) { tg.close(); } else { document.body.innerHTML = '<h2>✅ برای تایید نهایی به چت تلگرام برگردید.</h2>'; }
  } catch (e) {
    if (tg) tg.MainButton.hideProgress();
...
@app.get('/miniapp/manual_trade')
def miniapp_manual_trade_page():
    return _MINIAPP_MANUAL_TRADE_HTML, 200, {'Content-Type': 'text/html; charset=utf-8'}


@app.post('/miniapp/api/manual_trade')
def miniapp_api_manual_trade():
    body = request.get_json(silent=True) or {}
    init_data = body.get('initData', '')
    user = _validate_telegram_webapp_initdata(init_data)
    ...
    sym = re.sub(r'[^A-Z0-9]', '', str(body.get('symbol', '')).upper())
    if not (2 <= len(sym) <= 12):
        return {'error': 'نماد نامعتبر است'}, 400
    live = latest_price(sym)
    if live is None:
        return {'error': 'قیمت این نماد در دسترس نیست'}, 400
    ...
    if side_raw in ('BUY', 'LONG'):
        ...
        return {'error': 'جهت معامله نامعتبر است'}, 400

    entry_raw = body.get('entry')
    try:
        entry = live if entry_raw in (None, '', 'market') else float(entry_raw)
        tp = float(body.get('tp'))
        sl = float(body.get('sl'))
    except Exception:
        return {'error': 'مقادیر عددی نامعتبر است'}, 400

    is_long = side == 'BUY'
    if not ((is_long and sl < entry < tp) or ((not is_long) and tp < entry < sl)):
        return {'error': f"نسبت SL/TP/ورود با جهت {'خرید' if is_long else 'فروش'} همخوانی ندارد"}, 400

    s = get_session(chat_id)
    s['_manual_tmp'] = {'symbol': sym, 'side': side, 'entry': entry, 'tp': tp, 'sl': sl}
    s['user_state'] = None
    save_session(chat_id)
    side_fa = 'خرید (Long)' if is_long else 'فروش (Short)'
    summary = (
        f"🖐 *خلاصه‌ی معامله دستی (از فرم)*\n"
        f"• نماد: `{sym}` ({side_fa})\n"
        f"• ورود: `{fmt(entry)}`\n"
        f"• حد سود: `{fmt(tp)}`\n"
        f"• حد ضرر: `{fmt(sl)}`\n\n"
...
def miniapp_manual_trade_url():
    if not MINIAPP_BASE_URL:
        return None
    return f'{MINIAPP_BASE_URL}/miniapp/manual_trade'
