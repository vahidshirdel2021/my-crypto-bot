...
def get_user_session(chat_id):
    if chat_id not in USER_SESSIONS:
        USER_SESSIONS[chat_id] = {
            ...
            "paper_positions": [],
            "closed_positions": [],
            "cooldowns": {},  # سیستم وقفه برای جلوگیری از ورود مجدد تکراری
            "user_state": None,
...
def send_telegram_photo(photo_bytes, caption="", chat_target=None, reply_markup=None):
    ...
    if reply_markup:
        ...
        data["reply_markup"] = json.dumps(get_bottom_menu_keyboard(session["is_bot_active"]))
...
def generate_and_send_trade_chart(chat_id, symbol, df, trade):
    try:
        ...
        plt.plot(df['close'].values[-50:], label='Close Price', color='#2196F3', linewidth=2)
        if 'ema20' in df:
            plt.plot(df['ema20'].values[-50:], label='EMA20', color='#FF9800', linestyle='--', alpha=0.8)
        if 'ema50' in df:
            plt.plot(df['ema50'].values[-50:], label='EMA50', color='#9C27B0', linestyle='--', alpha=0.8)
