...
# Diagnostic mode: deliberately bypasses strategy/entry filters to prove whether
# the execution pipeline can open a paper position. NEVER use for live trading.
UNFILTERED_TEST = os.environ.get('UNFILTERED_TEST', '0').strip().lower() in ('1','true','yes','on')
...
if UNFILTERED_TEST:
    logger.warning('UNFILTERED_TEST enabled: strategy entry filters are bypassed; execution diagnostics are active')
app = Flask(__name__)
STATE_LOCK = RLock()
DB_LOCK = RLock()
DATA_LOCK = RLock()
...
def _execute_trade_unlocked(chat_id,symbol,side,signal_price,sl,tp,reason='',generation=None,require_active=True,structural_tp=False):
    ...
    if any(str(p.get('setup_id') or '') == setup_id for p in s.get('paper_positions', [])):
        return _execution_block(chat_id, symbol, 'duplicate_position_setup', 'همان setup_id قبلاً در پوزیشن باز وجود دارد', trade_id)
    if setup_id in set(s.get('consumed_setups') or []):
        return _execution_block(chat_id, symbol, 'consumed_setup', 'setup_id قبلاً مصرف شده است', trade_id)
    if (require_active and not s['is_bot_active']) or s['daily_stopped']:
        return _execution_block(chat_id, symbol, 'state', 'ربات فعال نیست یا daily_stopped فعال است', trade_id)
    if not risk_guard(chat_id):
        return _execution_block(chat_id, symbol, 'risk_guard', 'risk_guard اجازه ورود نداد', trade_id)
    now=time.time(); cd=float(s['cooldowns'].get(symbol,0))
    if now<cd:
        return _execution_block(chat_id, symbol, 'cooldown', f'cooldown فعال است تا {cd}', trade_id)
    s['cooldowns'].pop(symbol,None)
    if level_key and level_key in s.get('traded_levels', {}):
        return _execution_block(chat_id, symbol, 'traded_level', 'سطح معامله قبلاً معامله شده است', trade_id)
    ...
    if not is_dynamic_strategy and s['filters'].get('no_short_filter') and 'SELL' in side:
        return _execution_block(chat_id, symbol, 'no_short_filter', 'فیلتر no_short فعال است', trade_id)
    if not is_dynamic_strategy and s['filters'].get('no_buy_filter') and 'BUY' in side:
        return _execution_block(chat_id, symbol, 'no_buy_filter', 'فیلتر no_buy فعال است', trade_id)
    if s['max_open_positions']>0 and len(s['paper_positions'])>=s['max_open_positions']:
        return _execution_block(chat_id, symbol, 'max_open_positions', f"ظرفیت پوزیشن‌ها پر است: {len(s['paper_positions'])}/{s['max_open_positions']}", trade_id)
    if any(p['symbol']==symbol for p in s['paper_positions']):
        return _execution_block(chat_id, symbol, 'symbol_already_open', 'برای این نماد پوزیشن باز وجود دارد', trade_id)
    ...
    if (not UNFILTERED_TEST) and (not _same_direction_guard_allows(s, side, time.time(), quality_score=quality_score, planned_rr=planned_rr)):
        return _execution_block(chat_id, symbol, 'same_direction_guard', 'سقف پوزیشن‌های هم‌جهت اجازه ورود نداد', trade_id)
    ...
    if margin<=0:
        return _execution_block(chat_id, symbol, 'safe_size', f'محاسبه حجم معامله نامعتبر شد: {amount_or_reason}', trade_id)
    ...
    if (not UNFILTERED_TEST) and (not _passes_min_risk_to_fee_ratio(risk_usdt, fee_estimate, MIN_RISK_TO_FEE_RATIO)):
        return _execution_block(chat_id, symbol, 'risk_fee_ratio', f'نسبت ریسک به کارمزد کافی نیست: risk={risk_usdt:.6f} fee={fee_estimate:.6f}', trade_id)
    ...
    if s['trading_mode']=='REAL':
        ...
        available = float(s['paper_balance']) - reserved_margin(s)
        if available < margin:
            return _execution_block(chat_id, symbol, 'paper_balance', f'موجودی Paper کافی نیست: available={available:.6f} margin={margin:.6f}', trade_id)
        ...
        logger.info('ENTRY_EXEC_DIAG chat=%s symbol=%s stage=PAPER_OPENED entry=%s sl=%s tp=%s margin=%s amount=%s trade_id=%s', chat_id, symbol, price, sl, tp, margin, trade['amount'], trade_id)
        s['_last_execute_diag'] = {'status':'PAPER_OPENED','symbol':symbol,'stage':'paper_opened','reason':'پوزیشن Paper با موفقیت ثبت شد','trade_id':trade_id,'ts':time.time()}
...
def _execution_block(chat_id, symbol, stage, reason, trade_id=None):
    """Record the exact execution gate that rejected an entry.

    Diagnostic mode uses this to distinguish signal generation from paper
    execution failures. The normal boolean API is preserved.
    """
    msg = str(reason or 'نامشخص')
    try:
        s = get_session(chat_id)
        s['_last_execute_diag'] = {
        ...
        pass
    logger.warning('ENTRY_EXEC_DIAG chat=%s symbol=%s stage=%s reason=%s trade_id=%s',
                   chat_id, symbol, stage, msg, trade_id)
    return False
...
def execute_trade(chat_id,symbol,side,signal_price,sl,tp,reason='',structural_tp=False):
    ...
    s['_last_execute_diag'] = None
    save_session(chat_id)
    if not s['is_bot_active'] or s['daily_stopped']:
        return _execution_block(chat_id, symbol, 'precheck', 'ربات فعال نیست یا daily_stopped فعال است')
    lock=get_entry_lock(chat_id)
    with lock:
        s=get_session(chat_id)
        if not s['is_bot_active'] or s['daily_stopped'] or int(s.get('scan_generation',0)) != generation:
            return _execution_block(chat_id, symbol, 'state', 'وضعیت ربات/scan_generation هنگام ورود تغییر کرد')
...
async def _scan_symbol_impl(http,chat_id,symbol,regime=None):
    ...
    if (not UNFILTERED_TEST) and time.time() < float(s['cooldowns'].get(symbol,0)):
    ...
    # UNFILTERED_TEST: ignore all strategy filters and create a deterministic
    # candle-direction signal with an ATR-based 1:2 plan. This is diagnostic only.
    if UNFILTERED_TEST:
        try:
            price = float(latest_price(symbol) or primary['close'].iloc[-1])
            atr = float(primary['atr'].iloc[-2]) if 'atr' in primary.columns and pd.notna(primary['atr'].iloc[-2]) else price * 0.005
            atr = max(abs(atr), price * 0.002)
            prev_close = float(primary['close'].iloc[-2])
            side = 'BUY' if price >= prev_close else 'SELL'
            risk_dist = atr
            if side == 'BUY':
                sl = price - risk_dist; tp = price + 2.0 * risk_dist
            else:
                sl = price + risk_dist; tp = price - 2.0 * risk_dist
            reason = f'UNFILTERED_TEST | {side} | Quality=100 | RR=2.00 | no entry filters'
            ok = execute_trade(chat_id, symbol, 'BUY (Long)' if side == 'BUY' else 'SELL (Short)', price, sl, tp, reason)
            diag = get_session(chat_id).get('_last_execute_diag') or {}
            if ok:
                logger.info('UNFILTERED_TEST_RESULT chat=%s symbol=%s status=PAPER_OPENED side=%s entry=%s sl=%s tp=%s', chat_id, symbol, side, price, sl, tp)
                return _entry_diag_result(chat_id, symbol, 'entry_opened', f'{reason} | PAPER_OPENED', 'unfiltered_test', side, diagnostics=diag)
            block_reason = diag.get('reason') or 'دلیل اجرای معامله ثبت نشده است'
            block_stage = diag.get('stage') or 'execute'
            logger.warning('UNFILTERED_TEST_RESULT chat=%s symbol=%s status=EXECUTION_BLOCKED stage=%s reason=%s', chat_id, symbol, block_stage, block_reason)
            return _entry_diag_result(chat_id, symbol, 'execute_blocked', f'{reason} | EXECUTION_BLOCKED | stage={block_stage} | reason={block_reason}', 'unfiltered_test', side, diagnostics=diag)
        except Exception as exc:
            logger.exception('UNFILTERED_TEST failed chat=%s symbol=%s', chat_id, symbol)
            return _entry_diag_result(chat_id, symbol, 'test_error', f'UNFILTERED_TEST error: {type(exc).__name__}: {exc}', 'unfiltered_test')
