...
def get_bottom_menu_keyboard(is_active=False):
    # Telegram bots cannot place a button inside the native text-input field.
    # Use one compact reply-keyboard button as the closest supported UX.
    return {"keyboard": [[{"text": "☰ منو"}]], "resize_keyboard": True, "one_time_keyboard": False, "is_persistent": True}
...
def get_main_menu_keyboard(active):
    return {"inline_keyboard": [
        [{"text": "🔴 توقف اسکن" if active else "🟢 شروع اسکن", "callback_data": "/toggle_active"}],
        [{"text": "📊 وضعیت بازار", "callback_data": "/market_report"}, {"text": "🤖 تحلیل هوشمند بازار", "callback_data": "/ai_analyze"}],
        [{"text": "🤖 تنظیمات هوش مصنوعی", "callback_data": "/ai_settings"}],
        [{"text": "⚙️ تنظیمات معامله", "callback_data": "/check_wizard"}],
        [{"text": "🔍 تحلیل ارز", "callback_data": "/analyze_single"}, {"text": "📊 استراتژی", "callback_data": "/strategies_menu"}],
        [{"text": "⚙️ فیلترها", "callback_data": "/filters_menu"}, {"text": "🎛️ پارامترها", "callback_data": "/params_menu"}],
        [{"text": "📋 واچ‌لیست", "callback_data": "/manage_watchlist"}, {"text": "❌ بستن همه", "callback_data": "/close_all_prompt"}],
        [{"text": "🔄 پوزیشن‌ها", "callback_data": "/open_positions"}, {"text": "📈 عملکرد", "callback_data": "/performance"}],
    ]}


def get_ai_settings_keyboard():
    return {"inline_keyboard": [
        [{"text": "🟢 Gemini", "callback_data": "/ai_gemini"}, {"text": "🔵 OpenAI", "callback_data": "/ai_openai"}],
        [{"text": "🔄 بررسی اتصال", "callback_data": "/ai_status"}],
        [{"text": "🏠 منوی اصلی", "callback_data": "/menu"}],
    ]}
