...
def get_bottom_menu_keyboard(is_active=False, is_open=True):
    ...
    # دکمه‌ی دوم به‌جای کلید ثابت «پیگیری پوزیشن‌ها»، وضعیت زنده‌ی اسکن بازار را
    # نشان می‌دهد (سبز = فعال، قرمز = خاموش) و با تپ‌کردن، اسکن را toggle می‌کند.
    scan_status_text = "🟢 اسکن بازار فعال" if is_active else "🔴 اسکن بازار خاموش"
    return {
        "keyboard": [
            [{"text": "📊 وضعیت بازار"}, {"text": scan_status_text}],
